...
// const multiBuyOptionsTwo: MultiBuyOption[] = [
//     { qty: 5, discount: 0.05 },
//     { qty: 15, discount: 0.15 },
//     { qty: 20, discount: 0.50 },
// ];
...
export default function TicketForm({ product, onAfterSubmit }: { product: CompetitionProduct, onAfterSubmit?: () => void }) {
  ...
  const getMultiBuyDiscount = useCallback((): MultiBuyOption[] => {
    if (product.multi_buy_discounts) {

      return Object.entries(product.multi_buy_discounts).map(([qty, discount]) => ({
        qty: parseInt(qty, 10),
        discount: discount / 100, // convert percentage to decimal
      }));
    }

    return multiBuyOptions;
  }, [product.id]);
      ...
      <form className="cart" onSubmit={handleSubmit}>
        ...
        <div className="answers">
          ...
          <ul className="answers-list">
            {getMultiBuyDiscount().map(({ qty, discount }) => {
              ...
              return (
                <li
                  key={qty}
                  data-value={qty}
                  onClick={() => handleMultiBuySelect(qty)}
