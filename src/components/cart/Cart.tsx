...
export default function Basket() {
  ...
  const [rnPlatform, setRnPlatform] = useState<string | null>(null);

useEffect(() => {
  const id = setTimeout(() => {
    const rnPlatform = (window as any).__RN_PLATFORM__;
    
    
    if (rnPlatform) {
        console.log('React Native Platform', rnPlatform);
      setRnPlatform(rnPlatform);
    } else {
    ...
    //     setRnPlatform("android");
    //   } else if (/iphone|ipad|ipod/i.test(ua)) {
    //     setRnPlatform("ios");
    //   } else {
    //     setRnPlatform("browser");
    //   }
    }
  }, 100);
  ...
  const renderCartItems = useMemo(() => {
    if (cart && cart.ignored_coupons && cart.ignored_coupons.length > 0) {
      if (cart.ignored_coupons.length === 1) {
        setNotice(
          `⚠️ Coupon "${cart.ignored_coupons[0].code}" ignored: ${cart.ignored_coupons[0].message}`,
        );
      } else {
        setNotice(
          `⚠️ ${cart.ignored_coupons.map((c) => `"${c.code}" - ${c.message}`).join(", ")}`,
        );
      }
    }
            ...
            <td className="product-quantity" data-title="Quantity">
              <div className="quantity">
                <label
                  ...
                  htmlFor={`quantity_${item.product_id}`}
                >
                  {item.name} quantity
                </label>
                <CartQuantityInput
